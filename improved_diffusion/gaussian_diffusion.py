...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if t[0].item() < 2000:
            ...
            model_t = t
        model_output = model(x, self._scale_timesteps(model_t), **model_kwargs)
    ...
    def p_sample_loop_from_t(
        ...
        """
        final = None
        for sample in self.p_sample_loop_from_t_progressive(
            model,
            shape,
            x_t=x_t,
            t_start=t_start,
    ...
    def p_sample_loop_from_t_progressive(
        self,
        model,
        shape,
        x_t=None,
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, save_forward_clean_sample=False):
        ...
        # differentiable_params = [p for p in model.model.parameters() if p.requires_grad]
        ...
        # print(differentiable_params[0])
        # input("check")

        return terms

    def training_losses_for_gm(self, model, x_start, t, x_start_source, model_kwargs=None, noise=None, save_forward_clean_sample=False):
        """
        Compute training losses for a single timestep.
